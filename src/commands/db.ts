...
export const dbCommand = new Command('db')
  ...
  .addCommand(
    new Command('test')
      ...
      .argument('<environment>', 'Environment to test (dev, uat, pprd, prod, local)')
...
async function testDatabaseConnection(environment: string): Promise<void> {
  ...
  console.log(chalk.gray(`  Password: ${'*'.repeat(config.password?.length || 0)}`));
  ...
  try {
    ...
    if (connectionTest) {
      ...
      try {
        const tableCount = await DatabaseOperations.getEnvironmentTableCount(environment);
        console.log(chalk.green(`✓ Database access verified (${tableCount} tables found)`));
      } catch (queryError) {
        console.log(chalk.yellow('⚠ Connection established but database query failed'));
        console.log(chalk.gray(`  Query error: ${queryError instanceof Error ? queryError.message : 'Unknown error'}`));
      }
    } else {
      ...
      if (environment === 'local') {
        console.log(chalk.gray('  5. For DDEV: Make sure your project is running (ddev start)'));
        console.log(chalk.gray('  6. For DDEV: Check connection details with "ddev describe"'));
      }
    ...
    console.log(chalk.red(`Error: ${testError instanceof Error ? testError.message : 'Unknown error'}`));
    ...
    if (errorMessage.includes('Unknown MySQL server host')) {
      ...
      if (environment === 'local') {
        console.log(chalk.yellow('   For DDEV, the host should be something like "ddev-myproject-db"'));
      }
    } else if (errorMessage.includes("Can't connect") && errorMessage.includes('socket')) {
      console.log(chalk.yellow('\n💡 This usually means the MySQL server is not running or wrong port.'));
      if (environment === 'local') {
        console.log(chalk.yellow('   For DDEV, make sure your project is started with "ddev start"'));
      }
...
function listDatabaseEnvironments(): void {
  ...
  environments.forEach((env) => {
    if (Config.hasRequiredEnvironmentConfig(env)) {
      ...
      console.log(chalk.gray(`    ${config.host}${config.port ? ':' + config.port : ''} → ${config.database}`));
      hasConfigured = true;
    } else {
  ...
  if (!hasConfigured) {
    ...
    console.log(chalk.cyan('Run "wfuwp config wizard" to set up your environments.'));
  }
  ...
  if (Config.hasRequiredMigrationConfig()) {
    ...
    console.log(chalk.gray(`    ${migrationConfig.host} → ${migrationConfig.database}`));
  } else {
