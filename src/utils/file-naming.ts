...
export interface SqlFileOptions {
  siteId: string;
  environment: string;
  purpose: 'initial-export' | 'backup-export' | 'rename-export' | 'migrated-export';
  siteName?: string;
  date?: Date;
}
...
export class FileNaming {
  ...
  static parseFilename(filename: string): Partial<SqlFileOptions> | null {
    ...
    if (!year || !day || !month || !siteId || !environment || !purpose || !siteName) {
      return null;
    }
   ...
   */
  static async getSiteName(siteId: string, environment: string): Promise<string> {
    try {
      ...
      try {
        ...
        if (lines.length > 0 && lines[0]) {
          ...
          if (domain && domain !== 'domain') { // Skip header row
            // Extract meaningful name from domain
            let siteName = domain.replace(/\.(wfu\.edu|pprd\.wfu\.edu|dev\.wfu\.edu|uat\.wfu\.edu)$/, '');
            siteName = siteName.replace(/^(www\.|aws\.)/, '');
            
            // If path has meaningful info, include it
            if (path && path !== '/' && path !== '/wp/') {
              const pathName = path.replace(/^\//, '').replace(/\/$/, '').replace(/\/wp$/, '');
              if (pathName) {
        ...
        console.warn(`Warning: Could not get site name from database: ${dbError instanceof Error ? dbError.message : 'Unknown error'}`);
      }
    } catch (error) {
